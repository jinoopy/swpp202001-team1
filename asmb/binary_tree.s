start insert 2:
.entry:
  r6 = mul arg1 1 64 
  r5 = mul 0 1 64 
  br .while.cond 
.while.cond:
  r2 = load 8 r6 0 
  r1 = icmp ugt r2 arg2 64 
  br r1 .if.then .if.else 
.if.then:
  r1 = mul r6 1 64 
  r2 = add r1 8 64 
  r1 = mul r2 1 64 
  r1 = load 8 r1 0 
  r3 = icmp eq r1 0 64 
  r2 = mul r1 1 64 
  r1 = mul r5 1 64 
  r4 = mul 1 1 1 
  br r3 .if.then2 .cleanup 
.if.then2:
  r3 = mul r2 1 64 
  r2 = malloc 24 
  r1 = mul r2 1 64 
  store 8 arg2 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 8 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 16 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  store 8 r1 r3 0 
  r2 = mul r6 1 64 
  r4 = mul 0 1 1 
  r1 = mul 1 1 64 
  br .cleanup 
.if.else:
  r3 = icmp ult r2 arg2 64 
  r2 = mul r6 1 64 
  r4 = mul 0 1 1 
  r1 = mul 0 1 64 
  br r3 .if.then7 .cleanup 
.if.then7:
  r1 = mul r6 1 64 
  r2 = add r1 16 64 
  r1 = mul r2 1 64 
  r1 = load 8 r1 0 
  r3 = icmp eq r1 0 64 
  r2 = mul r1 1 64 
  r1 = mul r5 1 64 
  r4 = mul 1 1 1 
  br r3 .if.then10 .cleanup 
.if.then10:
  r3 = mul r2 1 64 
  r2 = malloc 24 
  r1 = mul r2 1 64 
  store 8 arg2 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 8 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 16 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  store 8 r1 r3 0 
  r2 = mul r6 1 64 
  r4 = mul 0 1 1 
  r1 = mul 1 1 64 
  br .cleanup 
.cleanup:
  r5 = mul r1 1 64 
  r6 = mul r2 1 64 
  br r4 .while.cond .cleanup19 
.cleanup19:
  ret r1 
end insert

start adjust 1:
.entry:
  r1 = mul arg1 1 64 
  r1 = add r1 8 64 
  r2 = load 8 r1 0 
  r1 = icmp eq r2 0 64 
  r4 = mul r2 1 64 
  br r1 .if.then .while.cond 
.if.then:
  r1 = mul arg1 1 64 
  r1 = add r1 16 64 
  r1 = load 8 r1 0 
  r2 = mul r1 1 64 
  br .cleanup 
.while.cond:
  r1 = mul r4 1 64 
  r1 = add r1 16 64 
  r2 = load 8 r1 0 
  r1 = icmp eq r2 0 64 
  r3 = mul r4 1 64 
  r4 = mul r2 1 64 
  br r1 .while.end .while.cond 
.while.end:
  r1 = icmp eq r3 0 64 
  r2 = mul r4 1 64 
  br r1 .cleanup .if.then6 
.if.then6:
  r1 = mul r4 1 64 
  r1 = add r1 8 64 
  r2 = load 8 r1 0 
  r1 = mul r3 1 64 
  r1 = add r1 16 64 
  store 8 r2 r1 0 
  r2 = mul r4 1 64 
  br .cleanup 
.cleanup:
  ret r2 
end adjust

start remove 2:
.entry:
  r1 = load 8 arg1 0 
  r4 = icmp eq r1 arg2 64 
  r3 = mul r1 1 64 
  r8 = mul arg1 1 64 
  r2 = mul 0 1 64 
  r1 = mul 0 1 64 
  br r4 .cleanup24 .while.cond 
.while.cond:
  r1 = icmp ugt r3 arg2 64 
  br r1 .if.then2 .if.else 
.if.then2:
  r1 = mul r8 1 64 
  r1 = add r1 8 64 
  r7 = mul r1 1 64 
  r6 = load 8 r1 0 
  r1 = icmp eq r6 0 64 
  r3 = mul r8 1 64 
  r4 = mul 1 1 32 
  r2 = mul 0 1 64 
  br r1 .cleanup .if.end5 
.if.end5:
  r1 = load 8 r6 0 
  r1 = icmp eq r1 arg2 64 
  r3 = mul r6 1 64 
  r4 = mul 2 1 32 
  br r1 .if.then7 .cleanup 
.if.then7:
  r1 = mul r6 1 64 
  r1 = add r1 8 64 
  r1 = load 8 r1 0 
  r2 = icmp eq r1 0 64 
  r4 = mul r1 1 64 
  br r2 .if.then.i .while.cond.i 
.if.then.i:
  r1 = mul r6 1 64 
  r1 = add r1 16 64 
  r1 = load 8 r1 0 
  br .adjust.exit 
.while.cond.i:
  r1 = mul r4 1 64 
  r1 = add r1 16 64 
  r2 = load 8 r1 0 
  r1 = icmp eq r2 0 64 
  r3 = mul r4 1 64 
  r4 = mul r2 1 64 
  br r1 .while.end.i .while.cond.i 
.while.end.i:
  r5 = icmp eq r3 0 64 
  r1 = mul r4 1 64 
  br r5 .adjust.exit .if.then6.i 
.if.then6.i:
  r1 = mul r4 1 64 
  r1 = add r1 8 64 
  r2 = load 8 r1 0 
  r1 = mul r3 1 64 
  r1 = add r1 16 64 
  store 8 r2 r1 0 
  r1 = mul r4 1 64 
  br .adjust.exit 
.adjust.exit:
  store 8 r1 r7 0 
  r1 = mul r6 1 64 
  free r1 
  r3 = mul r8 1 64 
  r4 = mul 1 1 32 
  r2 = mul 1 1 64 
  br .cleanup 
.if.else:
  r1 = icmp ult r3 arg2 64 
  r3 = mul r8 1 64 
  r4 = mul 0 1 32 
  br r1 .if.then11 .cleanup 
.if.then11:
  r1 = mul r8 1 64 
  r1 = add r1 16 64 
  r7 = mul r1 1 64 
  r6 = load 8 r1 0 
  r1 = icmp eq r6 0 64 
  r3 = mul r8 1 64 
  r4 = mul 1 1 32 
  r2 = mul 0 1 64 
  br r1 .cleanup .if.end15 
.if.end15:
  r1 = load 8 r6 0 
  r1 = icmp eq r1 arg2 64 
  r3 = mul r6 1 64 
  r4 = mul 2 1 32 
  br r1 .if.then17 .cleanup 
.if.then17:
  r1 = mul r6 1 64 
  r1 = add r1 8 64 
  r1 = load 8 r1 0 
  r2 = icmp eq r1 0 64 
  r4 = mul r1 1 64 
  br r2 .if.then.i4 .while.cond.i10 
.if.then.i4:
  r1 = mul r6 1 64 
  r1 = add r1 16 64 
  r1 = load 8 r1 0 
  br .adjust.exit19 
.while.cond.i10:
  r1 = mul r4 1 64 
  r1 = add r1 16 64 
  r2 = load 8 r1 0 
  r1 = icmp eq r2 0 64 
  r3 = mul r4 1 64 
  r4 = mul r2 1 64 
  br r1 .while.end.i13 .while.cond.i10 
.while.end.i13:
  r5 = icmp eq r3 0 64 
  r1 = mul r4 1 64 
  br r5 .adjust.exit19 .if.then6.i16 
.if.then6.i16:
  r1 = mul r4 1 64 
  r1 = add r1 8 64 
  r2 = load 8 r1 0 
  r1 = mul r3 1 64 
  r1 = add r1 16 64 
  store 8 r2 r1 0 
  r1 = mul r4 1 64 
  br .adjust.exit19 
.adjust.exit19:
  store 8 r1 r7 0 
  r1 = mul r6 1 64 
  free r1 
  r3 = mul r8 1 64 
  r4 = mul 1 1 32 
  r2 = mul 1 1 64 
  br .cleanup 
.cleanup:
switch r4 0 .while.cond.backedge 2 .while.cond.backedge .cleanup24
.while.cond.backedge:
  r1 = load 8 r3 0 
  r8 = mul r3 1 64 
  r3 = mul r1 1 64 
  br .while.cond 
.cleanup24:
  ret r1 
end remove

start traverse 1:
.entry:
  r1 = icmp eq arg1 0 64 
  r4 = mul arg1 1 64 
  br r1 .return .if.end 
.if.end:
  r3 = load 8 r4 0 
  r1 = mul r4 1 64 
  r1 = add r1 8 64 
  r2 = load 8 r1 0 
  r1 = mul r4 1 64 
  r1 = add r1 16 64 
  r1 = load 8 r1 0 
  call traverse r2 
  call write r3 
  r2 = icmp eq r1 0 64 
  r4 = mul r1 1 64 
  br r2 .return .if.end 
.return:
  ret 0 
end traverse

start main 0:
.entry:
  r2 = malloc 24 
  r9 = mul r2 1 64 
  r1 = call read 
  store 8 r1 r9 0 
  r1 = mul r2 1 64 
  r1 = add r1 8 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 16 64 
  store 8 0 r1 0 
  r8 = call read 
  r1 = icmp eq r8 0 64 
  r7 = mul 0 1 64 
  br r1 .for.cond.cleanup .for.body 
.for.cond.cleanup:
  call traverse r9 
  ret 0 
.for.body:
  r1 = call read 
  r6 = call read 
  r1 = icmp eq r1 0 64 
  r5 = mul r9 1 64 
  br r1 .while.cond.i .if.else 
.while.cond.i:
  r2 = load 8 r5 0 
  r1 = icmp ugt r2 r6 64 
  br r1 .if.then.i .if.else.i 
.if.then.i:
  r1 = mul r5 1 64 
  r3 = add r1 8 64 
  r1 = mul r3 1 64 
  r1 = load 8 r1 0 
  r2 = icmp eq r1 0 64 
  r4 = mul 1 1 1 
  br r2 .if.then2.i .cleanup.i 
.if.then2.i:
  r2 = malloc 24 
  r1 = mul r2 1 64 
  store 8 r6 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 8 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 16 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  store 8 r1 r3 0 
  r1 = mul r5 1 64 
  r4 = mul 0 1 1 
  br .cleanup.i 
.if.else.i:
  r2 = icmp ult r2 r6 64 
  r1 = mul r5 1 64 
  r4 = mul 0 1 1 
  br r2 .if.then7.i .cleanup.i 
.if.then7.i:
  r1 = mul r5 1 64 
  r2 = add r1 16 64 
  r1 = mul r2 1 64 
  r1 = load 8 r1 0 
  r3 = icmp eq r1 0 64 
  r4 = mul 1 1 1 
  br r3 .if.then10.i .cleanup.i 
.if.then10.i:
  r3 = mul r2 1 64 
  r2 = malloc 24 
  r1 = mul r2 1 64 
  store 8 r6 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 8 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  r1 = add r1 16 64 
  store 8 0 r1 0 
  r1 = mul r2 1 64 
  store 8 r1 r3 0 
  r1 = mul r5 1 64 
  r4 = mul 0 1 1 
  br .cleanup.i 
.cleanup.i:
  r5 = mul r1 1 64 
  br r4 .while.cond.i .if.end 
.if.else:
  r1 = call remove r9 r6 
  br .if.end 
.if.end:
  r1 = add r7 1 64 
  r2 = icmp eq r1 r8 64 
  r7 = mul r1 1 64 
  br r2 .for.cond.cleanup .for.body 
end main

