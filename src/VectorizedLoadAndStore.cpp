...
void VectorizedLoadAndStorePass::vectorize(BasicBlock& BB) {
	...
	//IMPORTANT:
	//we assume that no equal instruction exists in BB by GVN / GCSE passes.
	//e.g. %x = gep %ptr, 0
	...
	{
		map<Instruction *, unsigned int> orderIndvars;
		...
		//모든 size에 대해서,
		for (unsigned int bits = 8; bits <= 32; bits << 1)
		{
			...
			{
				//finds the most insignificant index
				...
				{
					//order the GEPs
					assert(orderGEPs[orderIndvars[last_index]] == nullptr && "equal instruction error; refer to the comment in VectorizedLoadAndStorePass::vectorize()");
					orderGEPs[orderIndvars[last_index]] = GEP;
				}
			}

			//group load and store if possible
			for (int i = 0; i <= orderGEPs.size() - 2; i++)
			{
	...
	for(Instruction& I : BB) {
		...
		if(addI->getOpcode()==Instruction::Add) {
			...
			if(!C1 && C1->getUniqueInteger()==1) {
				...
				for(auto it : insts) {
					if(it.size()>0 && it[0] == op2) indvar1 = &it;
					else if(it.size()>0 && it[0] == addI) indvar2 = &it;
				}
				//append %y to %x, so that %y comes later than %x in single list.
				indvar1->insert(indvar1->begin(), indvar2->begin(), indvar2->end());
			}

			//if op2 is constant 1,
			//e.g. %y = add %x, 1
			Constant* C2 = dyn_cast<Constant>(op2);
			if(!C2 && C2->getUniqueInteger()==1) {
				//find a list that begins with operand(==%x) and result(==%y).
				vector<Instruction*> *indvar1, *indvar2;
				for(auto it : insts) {
					if(it.size()>0 && it[0] == op1) indvar1 = &it;
					else if(it.size()>0 && it[0] == addI) indvar2 = &it;
				}
				//append %y to %x, so that %y comes later than %x in single list.
				indvar1->insert(indvar1->begin(), indvar2->begin(), indvar2->end());
...
vector<GetElementPtrInst *> VectorizedLoadAndStorePass::findGEPs(BasicBlock& BB, unsigned int size) {
...
void VectorizedLoadAndStorePass::vectorizeLoads(GetElementPtrInst*, LoadInst*, LoadInst*, BasicBlock&, unsigned int) {
	//TODO
	//Merge and replace two LoadInsts given to one.
	//the merged load should be done in the result of GEP.
}

void VectorizedLoadAndStorePass::vectorizeStores(GetElementPtrInst*, StoreInst*, StoreInst*, BasicBlock&, unsigned int) {
	//TODO
